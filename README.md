...
- 아이디어 명 : 내 손 안에 예산 지킴이 가계부 어플리케이션
...
- 계기 : 아직은 부모님한테 용돈을 받아서 생활하기 때문에 한 달 정해져 있는 용돈내에서 예산을 세우고 계획적으로 쓰는 소비 습관이 필요했습니다. 
 ...
 원래는 색으로 초과-빨강/적당-초록/여유-파랑 식으로 구분하여 한 눈에 봤을 때 식별이 가능해 편리했는데 업데이트 후 적당과 초과의 색을 동일하게 빨간색으로 설정하여 딱 봤을때 구분하기 어렵고, 
 정말 작게 써져있는 백분율 숫자를 봐야만 구분이 가능하고 전체적인 폰트 사이즈 자체도 작아져 가독성이 심하게 떨어져 불편함을 느꼈습니다. 또한 예산 설정을 아예 하지 않은 카테고리는 
 예산 항목에 반영 자체가 안되는 바람에 나중에 알고 봤더니 설정한 예산보다 더 많이 쓰는 경우도 발생하여 예산을 설정한 보람이 없어지기도 했습니다. 그래서 아예 다른 가계부 어플리케이션을 
 사용하려고 찾아봤는데 가계부 관련 앱은 많은데, '예산'을 정하고 '예산'만 집중적으로 관리하는 어플리케이션의 수는 상대적으로 매우 적은 편에 속했고 기존에 있던 예산 관리 앱이나 가계부 앱 내에
 있던 예산 관리 기능은 가독성이나 예산 관리 능력이 현저히 떨어지는 경우가 많아서 이용하는데 불편한 경우가 다소 있어 큰 메리트를 느끼지 못하여 그럼 '내가 쓰기 편한 예산 관리 집중 가계부 앱을
...
4) 기본적인 가계부 기능
5) 캘린더, 원형그래프, 카테고리별 분석 자료 등 다양한 지표의 자산 이용 정보 제공
6) 미션(챌린지나 퀘스트)참여 기능을 추가하여 사용자의 동기부여 촉진
7) 칭찬 및 잔소리 알림
...
이미지 바꾸기 버튼을 클릭하면 땃쥐가 돈땃쥐미로 변합니다(❁´◡`❁)
